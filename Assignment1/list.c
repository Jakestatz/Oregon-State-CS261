...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)){
    ...
    struct link* end, * temp;
    end = list->head;
    temp = list->head;

    if ((cmp(temp->val, val) == 0)) {
        list->head = list->head->next;
        free(temp);
        return;
    }

    while ((cmp(temp->val, val) != 0)) {
        end = temp;
        temp = temp->next;
    }
...
/*
 * This function should return the post (i.e. the 0-based "index") of the
 ...
 *
 * Return:
 *   This function should return the 0-based post of the first instance of
